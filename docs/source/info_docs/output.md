...
Po úpravě interního gridu přichází na řadu převod do výstupního formátu. Kvůli
...
podobě je tato transformace podporována pouze pro malou podmnožinu Factorio
...
Celý proces továrny je modulární a je možné používat libovolné recepty, které
nemají nic společného s Factoriem. Jediné aktuální omezení je právě export.
Poněvadž nepodporované recepty a objekty, které z nich vyjdou, nejdou vykreslit.
Tento problém má 2 řešení.
...
chleba bude vypadat jako ikona mapovaného objektu. (Zobrazování typů ve výrobnách
jde vypnout, takže by nebylo tolik matoucí.)

Druhý přístup je modifikovat open source projekt. Co jsem prozatím zjistil, tak
projekt je velmi modulární a objekty nejsou nijak hardcoded, ale používají
